  ...
  constructor: ({@model, @router, user}) ->
    ...
    @state = z.state {
      ...
      pastTrip: user.switchMap (user) =>
        if user
          @model.trip.getByUserIdAndType user.id, 'past' # TODO: per userId, don't embed route
        else
          RxObservable.of null
      futuretTrip: user.switchMap (user) =>
        if user
          @model.trip.getByUserIdAndType user.id, 'future' # TODO: per userId, don't embed route
        else
          RxObservable.of null
      $links: user.map (user) ->
